...
- Widening `scheduler_TRUSTED_SOURCES` past `loopback` for an off-box storage means any reverse proxy in front of storage **must** strip `Authorization` from public traffic — one that forwards it while dialing storage from inside the trusted range re-exposes `scheduler_AUTH` to the internet.
