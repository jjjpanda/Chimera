...
- Four cross-checks: `gateway_ON` must be `true`, since compose publishes only the gateway's ports and a blank `gateway_PORT` aborts `docker compose config` outright; `object_ON` requires `livestream_ON`; `command_COOKIE_SECURE` must be `true` on an HTTPS deploy at a non-loopback host, so a loopback `gateway_HOST` is never flagged; and `gateway_PORT` must be `80` when `certbot_ON=true`. Compose publishes only `gateway_PORT`, so an HTTP-01 challenge on any other port never arrives.
- No value may contain `#` — dotenv reads it as a comment. Checked on wizard answers and on values already in the file.
- The walk repeats until stable, since answering one key can unskip an earlier one. `livestream_ON`/`object_ON`, `gateway_HOST`/`command_COOKIE_SECURE`, and `certbot_ON`/`gateway_PORT` are re-asked as pairs — each is valid alone, so one pass would never revisit them. `gateway_ON` is re-asked alone, and turning it back on unskips the rest of the gateway keys on the next pass.
- EOF (Ctrl-D) at any prompt aborts: nothing is written and the run exits `1`. The walk only writes `.env` once, after it goes quiet, so an interrupted setup never leaves a half-filled file.
