...
- Two cross-checks: `command_COOKIE_SECURE` must be `true` on an HTTPS deploy, and `gateway_PORT` must be `80` when `certbot_ON=true`. Compose publishes only `gateway_PORT`, so an HTTP-01 challenge on any other port never arrives.
...
**At boot** — `validateEnvVars.js` re-runs both cross-checks and adds `*_URL` scheme and absolute-path checks. A build can pass preflight and still fail at boot.
