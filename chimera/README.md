...
- Only the loopback set arms an action. It skips DNS, TLS and anything proxying in front, so a dropped ISP link, a router with no NAT hairpin, or an expired certificate cannot reboot a recorder that is running fine. `compose.yml` publishes `gateway_PORT` on the host, and `gateway.js` exempts the health paths from the HTTPS redirect, so the probe reaches the service rather than a 302.
