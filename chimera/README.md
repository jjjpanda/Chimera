...
- Cross-checks: `object_ON` requires `livestream_ON`; `command_COOKIE_SECURE` must be `true` on an HTTPS deploy at a non-loopback host, so a loopback `gateway_HOST` is never flagged; `command_COOKIE_SECURE` must be `false` when `gateway_HOST` carries an explicit `http://` prefix and neither `gateway_HTTPS_Redirect` nor `certbot_ON` is `true`; and `gateway_PORT` must be `80` when `certbot_ON=true`. Compose publishes only `gateway_PORT`, so an HTTP-01 challenge on any other port never arrives.
