...
module.exports = {
        ...
        if (req.header('Cookie') != undefined) {
            ...
            if(bearerTokenCookie){
                ...
                jwt.verify(bearerToken.split("%20")[1], secretKey, (err, decoded) => {
                    if (err) {
                        res.redirect("/?loginForm")//res.status(401).send({ error: true, unauthorized: true });
                    } else {
                        next();
                    }
                });
    ...
    login: (req, res) => {
        ...
        let hash
        try{
            hash = fs.readFileSync(path.resolve(process.env.password_FILEPATH), {encoding:'utf8', flag:'r'});
        } catch(e) {
    ...
    register: (successCallback, errorCallback) => {
        let password
        try{
            password = fs.readFileSync(path.resolve(process.env.password_FILEPATH), {encoding:'utf8', flag:'r'});
        } catch(e) {
        ...
        fs.writeFileSync(path.resolve(process.env.password_FILEPATH), hash)
