...
- `tracker` — admin webhook alert in two tiers. **High-impact:** `/authorization/(login|setup|password|users|sessions)`, `/camera/<id>`, `/convert/(createVideo|createZip|cancelProcess|deleteProcess)`, `/file/(pathDelete|pathClean|pathAutoClean)`, `/livestream/restart`, `/object/(config|scan)`, `/task/(start|stop|destroy)`; `GET` on the users/sessions list endpoints is excluded. **Probe:** any path no service mounts, less the files browsers and crawlers fetch unprompted (`favicon.ico`, `robots.txt`, `sitemap.xml`, `apple-touch-icon*.png`). Every other path is silent. Each tier holds its own budget of 30 alerts globally and 10 per IP each minute, so a probe flood cannot silence high-impact alerts.
