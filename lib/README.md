...
- `rateLimiter(namespace)` — reservation-based rate limiter factory; returns `{ rateLimit, client }`. Shares failures across pm2 instances via `memory.client(namespace)` when `memory_ON=true`, falling back to a local store when disconnected or when a shared reservation's ack errors or times out (1s). `rateLimit({ windowMs, max, keyFn, releaseOnSuccess, throttleMs, skip })` wires the 429 response and sets `req.throttled` once a budget is spent (`auth.js` reads the flag to answer 429 on a wrong password). Without `throttleMs` a spent budget hard-blocks every later request; with it, one request per `throttleMs` passes through to the handler and the rest get 429. `skip(req)` returns a promise and bypasses the budget entirely when it resolves truthy. `releaseOnSuccess` refunds the reservation on a non-4xx finish — callers that must count every accepted call omit it.
...
- `languages` — shared JSON array of the valid language tags; the single source of truth for the `PUT /authorization/language` allow-list and the web app's picker.
