...
**Middleware**
- `auth` — session/JWT-cookie guard + `requireAdmin` RBAC; scheduler bypass requires all three of `scheduler_AUTH`, a path in `schedulableUrls`, and a `req.ip` matching `scheduler_TRUSTED_SOURCES` (proxy-addr list, defaults to `loopback`). Invalid values are rejected by preflight — `proxy-addr.compile` throws at import and would crash-loop every service.
