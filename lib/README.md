...
- `tracker` — admin webhook alert on an allowlist of high-impact paths only: `/authorization/(login|setup|password|users|sessions)`, `/camera/<id>`, `/convert/(createVideo|createZip|cancelProcess|deleteProcess)`, `/file/(pathDelete|pathClean|pathAutoClean)`, `/livestream/restart`, `/object/(config|scan)`, `/task/(start|stop|destroy)`. Every other path is silent, and `GET` on the users/sessions list endpoints is excluded. Capped at 30 alerts globally and 10 per IP each minute.
