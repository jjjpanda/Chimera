...
**Middleware**
- `auth` — session/JWT-cookie guard + `requireAdmin` RBAC; scheduler bypass requires `scheduler_AUTH` and a path in `schedulableUrls`. It also checks a socket peer address against `scheduler_TRUSTED_SOURCES` (proxy-addr list, defaults to `loopback`), but behind the gateway that peer is always loopback, so this is effectively two gates, not three — the trusted-source check only constrains traffic that reaches storage directly, which Docker already closes off by not publishing the port. Invalid values are rejected by preflight — `proxy-addr.compile` throws at import and would crash-loop every service.
...
- `rateLimiter(namespace)` — reservation-based rate limiter factory; returns `{ rateLimit, makeReserve, releaseOnSuccess, client }`. Shares failures across pm2 instances via `memory.client(namespace)` when `memory_ON=true`, falling back to a local in-process store when disconnected. `rateLimit(opts)` wires the 429 response and, only if `opts.releaseOnSuccess` is set, refunds the reservation on a non-4xx finish — callers that must count every accepted call (not just failures) omit it.
