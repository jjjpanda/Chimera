...
module.exports = {
	...
	passwordCheck: (memoryClient) => (req, res, next) => {
		const { password } = req.body
    
		memoryClient.emit('verifyPassword', password, (exists) => {
			if(exists){
				memoryClient.emit("deletePassword", password)
				next()
			}
			else{
	...
	generateLink: (memoryClient) => (req, res) => {
		const password = randomId.generate(50)
		memoryClient.emit('savePassword', password, () => {
