		...
		if (pool) {
			...
			try {
				const r = await pool.query(
					"WITH bumped AS (UPDATE sessions SET last_seen = NOW() WHERE jti = $2 RETURNING revoked) SELECT a.role, a.force_password_change, b.revoked FROM auth a LEFT JOIN bumped b ON TRUE WHERE a.username = $1",
