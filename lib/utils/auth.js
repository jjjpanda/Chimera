...
const makeAuthorize = (pool, { schedulableUrls = [], forcedChangeAllowed = [] } = {}) => (req, res, next) => {
	const unauthorized = () => respondUnauthorized(req, res)
	if (process.env.scheduler_AUTH && timingSafeCompare(req.headers.authorization, process.env.scheduler_AUTH) && schedulableUrls.includes(req.path) && isTrustedSource(req)) {
