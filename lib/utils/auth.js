...
const schedulerAuth = readSecret("scheduler_AUTH")
const jwt = require("jsonwebtoken")
const { compileTrustedSources } = require("./trustedSources.js")
...
const trustedSources = compileTrustedSources(process.env.scheduler_TRUSTED_SOURCES)
const isTrustedSource = (req) => trustedSources(req.socket?.remoteAddress, 0)
...
const makeAuthorize = (pool, { schedulableUrls = [], forcedChangeAllowed = [] } = {}) => (req, res, next) => {
	const unauthorized = () => respondUnauthorized(req, res)
	if (schedulerAuth && timingSafeCompare(req.headers.authorization, schedulerAuth) && schedulableUrls.includes(req.path) && isTrustedSource(req)) {
