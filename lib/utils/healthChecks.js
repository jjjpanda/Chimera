...
/**
 * The gateway's http listener on this machine, which compose publishes on the host.
 * Reaching it over loopback skips DNS, TLS and anything proxying in front, so a poll
 * failure means the stack itself, not the path a visitor takes to it.
 */
const localBase = () => {
...
healthChecks.localBase = localBase
healthChecks.isHealthPath = (path) => HEALTH_PATHS.has(path)
