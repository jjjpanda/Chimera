...
describe("makeAuthorize scheduler path", () => {
	...
	test("scheduler token on a schedulable url authorizes as admin", () => {
		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: process.env.scheduler_AUTH }, ip: "127.0.0.1" }
		const res = makeRes()
		const next = jest.fn()
	...
	test("scheduler token on a non-schedulable url is rejected", () => {
		const req = { method: "POST", path: "/convert/notAllowed", headers: { authorization: process.env.scheduler_AUTH }, ip: "127.0.0.1" }
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
	...
	test("wrong authorization value is rejected", () => {
		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: "wrong" }, ip: "127.0.0.1" }
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("a blank scheduler_AUTH does not let an empty authorization header bypass auth", () => {
		...
		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: "" }, cookies: {}, ip: "127.0.0.1" }
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
	...
	test("a correct scheduler token from a non-loopback address is rejected", () => {
		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: process.env.scheduler_AUTH }, cookies: {}, ip: "10.0.0.5" }
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("an IPv4-mapped IPv6 loopback address is still treated as loopback", () => {
		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: process.env.scheduler_AUTH }, ip: "::ffff:127.0.0.1" }
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).toHaveBeenCalled()
	...
	test("scheduler_TRUSTED_SOURCES widens the trusted range for an off-box storage deploy", () => {
		...
		process.env.scheduler_TRUSTED_SOURCES = original

		const req = { method: "POST", path: schedulableUrls[0], headers: { authorization: process.env.scheduler_AUTH }, ip: "10.1.2.3" }
		const res = makeRes()
		const next = jest.fn()
