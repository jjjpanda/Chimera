...
describe("makeAuthorize scheduler path", () => {
	...
	const schedReq = ({ path = schedulableUrls[0], authorization = process.env.scheduler_AUTH, headers = {}, ...rest } = {}) =>
		({ method: "POST", path, headers: { authorization, ...headers }, ...rest })
	const loopback = { remoteAddress: "127.0.0.1" }

	test("scheduler token on a schedulable url authorizes as admin", () => {
		const req = schedReq({ socket: loopback })
		const res = makeRes()
		const next = jest.fn()
	...
	test("scheduler token on a non-schedulable url is rejected", () => {
		const req = schedReq({ path: "/convert/notAllowed", socket: loopback })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
	...
	test("wrong authorization value is rejected", () => {
		const req = schedReq({ authorization: "wrong", socket: loopback })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("a blank scheduler_AUTH does not let an empty authorization header bypass auth", () => {
		...
		const req = schedReq({ authorization: "", cookies: {}, socket: loopback })
		const res = makeRes()
		const next = jest.fn()
		jest.isolateModules(() => {
			require("../utils/auth.js").createAuthorize(null, { schedulableUrls })(req, res, next)
		})
	...
	test("a correct scheduler token from a non-loopback address is rejected", () => {
		const req = schedReq({ cookies: {}, socket: { remoteAddress: "10.0.0.5" } })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("a correct scheduler token with no socket peer is rejected", () => {
		const req = schedReq({ cookies: {} })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("a forged X-Forwarded-For cannot make a remote client look loopback", () => {
		const req = schedReq({ headers: { "x-forwarded-for": "127.0.0.1" }, cookies: {}, ip: "127.0.0.1", socket: { remoteAddress: "203.0.113.9" } })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).not.toHaveBeenCalled()
	...
	test("an IPv4-mapped IPv6 loopback address is still treated as loopback", () => {
		const req = schedReq({ socket: { remoteAddress: "::ffff:127.0.0.1" } })
		const res = makeRes()
		const next = jest.fn()
		authorize(req, res, next)
		expect(next).toHaveBeenCalled()
	...
	test("scheduler_TRUSTED_SOURCES widens the trusted range for an off-box storage deploy", () => {
		...
		const req = schedReq({ socket: { remoteAddress: "10.1.2.3" } })
		const res = makeRes()
		const next = jest.fn()
