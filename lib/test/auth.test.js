...
describe("makeAuthorize jwt pool path", () => {
	...
	test("responds 500 when pool.query rejects", async () => {
		...
		expect(pool.query).toHaveBeenCalledWith("SELECT role, force_password_change FROM auth WHERE username = $1", ["bob"])
...
describe("makeAuthorize forced password change", () => {
	...
	const forcedPool = () => ({ query: jest.fn()
		.mockResolvedValueOnce({ rows: [{ role: "user", force_password_change: true }], rowCount: 1 })
		.mockResolvedValue({ rows: [{ revoked: false }], rowCount: 1 }) })
