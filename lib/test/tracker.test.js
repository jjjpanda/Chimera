...
describe("tracker", () => {
	...
	test("rate limits per IP rather than globally", () => {
		const { tracker, webhookAlert } = load()
		const next = jest.fn()

		for (let i = 0; i < 40; i++) tracker(mockReq({ ip: "1.1.1.1" }), {}, next)
	...
	test("emits a single suppression notice once an IP exceeds MAX, then stays silent", () => {
		const { tracker, webhookAlert } = load()
		const next = jest.fn()

		for (let i = 0; i < 35; i++) tracker(mockReq({ ip: "3.3.3.3" }), {}, next)

		expect(next).toHaveBeenCalledTimes(35)
		expect(webhookAlert).toHaveBeenCalledTimes(31)
		expect(webhookAlert.mock.calls[30][0]).toContain("suppressed")
	})

	test("resets the per-IP budget after the window elapses", () => {
		const { tracker, webhookAlert } = load()
		const next = jest.fn()

		for (let i = 0; i < 30; i++) tracker(mockReq({ ip: "9.9.9.9" }), {}, next)
		expect(webhookAlert).toHaveBeenCalledTimes(30)

		tracker(mockReq({ ip: "9.9.9.9" }), {}, next)
		expect(webhookAlert).toHaveBeenCalledTimes(31)
