...
describe("getDirectorySize", () => {
	...
	test("respects process.env.storage_DIR_CONCURRENCY", async () => {
		jest.useRealTimers()
		const originalEnv = process.env.storage_DIR_CONCURRENCY
		process.env.storage_DIR_CONCURRENCY = "2"
		...
		expect(size).toBe(5)
		expect(maxConcurrentStats).toBe(2)

		statSpy.mockRestore()
		if (originalEnv === undefined) {
			delete process.env.storage_DIR_CONCURRENCY
		} else {
			process.env.storage_DIR_CONCURRENCY = originalEnv
		}
	})
})
