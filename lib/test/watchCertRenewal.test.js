...
describe("watchCertRenewal", () => {
	...
	test("retries on the next tick when the restart fails", () => {
		pm2.restart
			.mockImplementationOnce((name, cb) => cb(new Error("pm2 busy")))
			.mockImplementationOnce((name, cb) => cb(new Error("pm2 busy")))
		watchCertRenewal(3, 4, TICK)
		fs.statSync.mockReturnValue({ mtimeMs: 2000 })
		jest.advanceTimersByTime(TICK)
		expect(pm2.restart).toHaveBeenCalledTimes(1)
		jest.advanceTimersByTime(TICK)
		expect(pm2.restart).toHaveBeenCalledTimes(2)
	})

	test("keeps retrying after the restart window closes if the first attempt failed", () => {
	...
	test("stops restarting once a restart succeeds", () => {
		pm2.restart.mockImplementationOnce((name, cb) => cb())
		watchCertRenewal(3, 4, TICK)
		fs.statSync.mockReturnValue({ mtimeMs: 2000 })
		jest.advanceTimersByTime(TICK)
		expect(pm2.restart).toHaveBeenCalledTimes(1)
