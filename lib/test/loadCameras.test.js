...
describe("cameraConfFiles", () => {
	...
	test("returns [] rather than rejecting when motion.conf is unreadable", async () => {
		const spy = jest.spyOn(console, "error").mockImplementation(() => {})
		require("fs").promises.readFile.mockRejectedValue(new Error("EACCES"))
		expect(await cameraConfFiles(7)).toEqual([])
		spy.mockRestore()
	})
})
