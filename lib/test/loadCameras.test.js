...
describe("resetCache", () => {
	const { resetCache } = require("../utils/loadCameras")
	...
	test("busts the 10s module cache so a deleted camera is not served", () => {
		process.env.NODE_ENV = "production"
		process.env.storage_MOTION_CONF_FILEPATH = "/etc/motion/motion.conf"
		const { readFileSync, readdirSync } = require("fs")
		readFileSync.mockImplementation((p) => {
			if (p === "/etc/motion/motion.conf") return "camera_dir /etc/motion/cameraconf\n"
			if (String(p).endsWith("gone.conf")) return "camera_id 1\ncamera_name gone\nnetcam_url rtsp://1.1.1.1/s\n"
			return ""
		})
		readdirSync.mockImplementation((p) => (p === "/etc/motion/cameraconf" ? ["gone.conf"] : []))

		resetCache()
		expect(loadCameras().map(c => c.name)).toEqual(["gone"])
		readdirSync.mockImplementation((p) => (p === "/etc/motion/cameraconf" ? [] : []))
		expect(loadCameras().map(c => c.name)).toEqual(["gone"])

		resetCache()
		expect(loadCameras()).toEqual([])
	})
})
