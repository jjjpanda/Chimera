...
describe("memory scheduledTasks lifecycle", () => {
	...
	test("createTask registers a disabled task without scheduling it so it stays listable and destroyable", () => {
		cron.validate.mockReturnValue(false)
		let ack
		tasks.createTask({ ...taskObject, cronString: "not a cron", disabled: true }, a => { ack = a })

		expect(cron.createTask).not.toHaveBeenCalled()
		expect(fakeTask.start).not.toHaveBeenCalled()
		expect(ack[taskObject.id]).toEqual(expect.objectContaining({ id: taskObject.id, disabled: true, running: false }))
	...
	test("createTask disabling an already scheduled id tears its cron down", () => {
		tasks.createTask(taskObject)
		tasks.createTask({ ...taskObject, disabled: true })
