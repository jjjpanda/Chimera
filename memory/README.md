...
- **converterProcesses** — cancel handles for in-flight mp4/zip jobs ([storage](../storage)). The handle is the ack of storage's `saveProcessEnder` emit, held in the client's `socket.acks` until this server answers it. `cancelProcess` answers `true` (cancel); `deleteProcessEnder` answers `false` (job already over) — it must answer, or storage keeps the closure, and the ffmpeg or archiver object it captures, for the life of the process. `disconnect` drops the handles unanswered, since the client clears its own acks on close. Storage gates both emits on `memory_ON` and `client.connected`, so with memory off or the socket still connecting, no ack is registered and no ender exists — nothing buffers for a later flush. A job started in that window stays uncancellable until the 24h orphan sweep reconciles it.
- **sessionSync** — broadcasts `sessionInvalidate`, `sessionInvalidateUser`, `sessionInvalidateAll` so every `AUTH` client's session cache stays in sync. Immediate cross-service revocation needs `memory_ON=true`; with it off, `invalidateSession`/`invalidateUser` clear only the calling process's cache, and other workers serve a revoked session from theirs for up to `SESSION_CACHE_MS` (5s, [../lib/utils/auth.js](../lib/utils/auth.js)).
