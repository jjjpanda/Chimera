...
const SideMenu = ({ index, mobile }) => {
	...
	const mobileTabs = [
		{ key: "route-1", icon: Scissors, title: t("nav.clip") },
		{ key: "route-2", icon: Video, title: t("nav.live") },
		{ key: "route-0", icon: Home, title: t("nav.home") },
		{ key: "route-3", icon: Rewind, title: t("nav.recordings") },
		{ key: "route-4", icon: Activity, title: t("nav.stats") },
	]

	const desktopTabs = [
		{ key: "route-0", icon: Home, title: t("nav.home") },
		{ key: "route-2", icon: Video, title: t("nav.live") },
		{ key: "route-1", icon: Scissors, title: t("nav.clipMaker") },
		{ key: "route-3", icon: Rewind, title: t("nav.recordings") },
		{ key: "route-4", icon: Activity, title: t("nav.stats") },
		{ key: "route-5", icon: CalendarClock, title: t("nav.schedule") },
		{ key: "route-7", icon: ScanEye, title: t("nav.objects") },
		{ key: "route-6", icon: Users, title: t("nav.admin") },
	].filter(tab => role === "admin" || !adminRoutes.has(tab.key))
			...
			<nav className="fixed inset-x-0 bottom-0 z-40 flex items-center justify-around border-t border-border bg-surface px-1 py-2">
				{mobileTabs.map(({ key, icon: Icon, title }) => {
					...
					return (
						...
						>
							{key === "route-0"
								? <img src="/res/logo.png" alt={t("nav.home")} className="size-6 object-contain" />
	...
	return (
		<aside className="sticky top-0 flex h-screen w-48 shrink-0 flex-col border-r border-border bg-surface">
			<div className="flex items-center gap-2 px-4 py-4">
				<img src="/res/logo.png" alt={t("common.appName")} className="h-8 w-8 object-contain" />
