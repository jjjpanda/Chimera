			...
			<Card className="h-full">
				...
				<CardContent>
					{loading ? (
						...
						<p className="py-4 text-center text-sm text-danger">{t("admin.loadUsersFailed")} <button onClick={fetchUsers} className="underline hover:opacity-80">{t("common.retry")}</button></p>
					) : (
							...
							{users.map(user => (
								<li key={user.username} className="flex items-center justify-between gap-2 py-2 min-w-0">
									<div className="flex flex-col min-w-0">
										...
										<span className="text-[11px] text-muted">{user.last_login ? moment(user.last_login).fromNow() : t("common.never")}</span>
									</div>
		...
		<div className="p-4">
			<Card className="bg-surface border-border">
				<CardHeader className="flex flex-row items-center justify-between pb-2">
					<CardTitle className="text-primary text-lg">{t("admin.users")}</CardTitle>
