...
test("a blocklisted password blocks submission and shows an error", () => {
	const trySetup = jest.fn()
	render(React.createElement(SetupForm, { trySetup, tokenRequired: true }))

	fillForm({ password: "PasswordPassword", confirmPassword: "PasswordPassword" })
	fireEvent.click(screen.getByText("Create Account"))

	expect(trySetup).not.toHaveBeenCalled()
	expect(screen.getByText("That password is too common. Choose something harder to guess.")).toBeTruthy()
})
